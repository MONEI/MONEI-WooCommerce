...
function parseChangelog(changelogContent) {
  ...
  for (const line of lines) {
    ...
    const versionMatch = line.match(/^##\s+(?:<small>)?(\d+\.\d+\.\d+)\s+\(([^)]+)\)(?:<\/small>)?/);
...
function generateReadmeTxt(template, changelog, metadata, version) {
  ...
  if (metadata.requiresAtLeast) {
    readme = readme.replace(/Requires at least: .+/g, `Requires at least: ${metadata.requiresAtLeast}`);
  }
  if (metadata.testedUpTo) {
    readme = readme.replace(/Tested up to: .+/g, `Tested up to: ${metadata.testedUpTo}`);
  }
  if (metadata.requiresPHP) {
    readme = readme.replace(/Requires PHP: .+/g, `Requires PHP: ${metadata.requiresPHP}`);
  }
  if (metadata.wcRequiresAtLeast) {
    readme = readme.replace(/WC requires at least: .+/g, `WC requires at least: ${metadata.wcRequiresAtLeast}`);
  }
  if (metadata.wcTestedUpTo) {
    readme = readme.replace(/WC tested up to: .+/g, `WC tested up to: ${metadata.wcTestedUpTo}`);
  }
...
function generateReadmeMd(readmeTxt) {
  ...
  readme = readme.replace(/^=\s+(v\d+\.\d+\.\d+\s+-\s+\d{4}-\d{2}-\d{2})\s+=$/gm, '### $1 ###');
...
function main() {
  try {
    ...
    console.log(`📝 Changelog limit: ${CONFIG.changelogLimit} versions\n`);
    ...
    const formattedChangelog = formatForReadme(versions, CONFIG.changelogLimit);

    // Generate readme files
    const readmeTxt = generateReadmeTxt(template, formattedChangelog, metadata, version);
