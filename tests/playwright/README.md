...
**Baselines are macOS renders, and CI compares Linux renders against them.** Same convention as monei-js: one set of PNGs, `-darwin` in the name, and a tolerance (`threshold` 0.3, `maxDiffPixelRatio` 0.1 in `playwright.config.js`) that absorbs the drift between the two. The page text is a Google Fonts webfont and renders the same everywhere; the MONEI card iframes use the system-ui stack, which is San Francisco on a Mac and Ubuntu or DejaVu on the runner, and no font install closes that. The tolerance is sized for it. A per-test bump needs a comment saying what drift it absorbs.
