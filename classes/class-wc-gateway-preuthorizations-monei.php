...
class WC_Gateway_Preuthorizations_Monei extends WC_Payment_Gateway {
	var $notify_url;
	/**
	...
	public function __construct() {
		global $woocommerce;
		$this->id = 'moneipreauth';
		$logo_url = $this->get_option( 'logo' );
		if ( ! empty( $logo_url ) ) {
			$logo_url   = $this->get_option( 'logo' );
			$this->icon = apply_filters( 'woocommerce_moneipreauth_icon', $logo_url );
		} else {
			$this->icon = apply_filters( 'woocommerce_moneipreauth_icon', plugins_url( basename( plugin_dir_path( __FILE__ ) ), basename( __FILE__ ) ) . '/assets/images/MONEI-logo.png' );
		}
		...
		$this->method_title         = __( 'MONEI Preauthorizations', 'monei' );
		$this->notify_url           = add_query_arg( 'wc-api', 'WC_Gateway_preuthorizations_monei', home_url( '/' ) );
		$this->notify_url_not_https = str_replace( 'https:', 'http:', add_query_arg( 'wc-api', 'WC_Gateway_preuthorizations_monei', home_url( '/' ) ) );
		...
		$this->init_settings();
		// Actions.
		add_action( 'valid_moneipreauth_standard_ipn_request', array( $this, 'successful_request' ) );
		add_action( 'woocommerce_receipt_moneipreauth', array( $this, 'receipt_page' ) );
	...
	function get_monei_args( $order ) {
		...
		$monei_args = apply_filters( 'woocommerce_moneipreauth_args', $monei_args );
		return $monei_args;
	}
	...
	function process_payment( $order_id ) {
		...
		$monei_adr         = 'https://api.monei.net/v1/payments';
	...
	*/
	function check_ipn_response() {
		...
		if ( 'SUCCEEDED' === $result ) {
			...
			do_action( 'valid_moneipreauth_standard_ipn_request', $data );
		} else {
...
function woocommerce_add_gateway_preauthorizations_monei_gateway( $methods ) {
	$methods[] = 'WC_Gateway_Preuthorizations_Monei';
	return $methods;
}
add_filter( 'woocommerce_payment_gateways', 'woocommerce_add_gateway_preauthorizations_monei_gateway' );